...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _destroy(self, instance):
        try:
            ...
            cur = db.cursor()
            cur.execute ("select host from  vms where uuid = " + "\'" + instance['uuid']+"\'")
            ...
            cur.execute ("delete from  vms where uuid = " + "\'" + instance['uuid']+"\'")
            cur.close()
            db.commit()
            db.close()
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        dnic = self.vif_driver.get_config(
                      instance, network_info[0], image_meta,
                      flavor, CONF.libvirt.virt_type)
        def randomMAC():
            mac = [ 0x00, 0x16, 0x3e,
        ...
        if shared.get('vlantag') != "empty" :
          dnic.net_type = "bridge"
          dnic.source_dev = "br-ant"
          dnic.source_mode = "private"
          dnic.model = "virtio"
          dnic.vporttype = "openvswitch"
          insname = instance['name']
          vlan = str(shared.get('vlantag'))
          mac = randomMAC()
          dnic.mac_addr = mac

          shared.set ('vlantag', 'empty')
          dnic.target_dev= "tap"+insname[10:]
          ...
          import urllib2
          total = config.get('Neighbor','number')
          for i in range(1, int(total)+1):
             current = "Neighbor"+str(i)
             ip = config.get(current, 'ip')
             urllib2.urlopen('http://'+ip+'/html/startflows')
          import MySQLdb
          host = str(shared.get('host'))
#If host is set its mean request is from some other cluster so we need to write to both local and remote database. If host is not set, the request is local and only local db is written.
          import socket # using it to validate host ip
          LOG.info('khayam')
          LOG.info(str(shared.get('host')))
          host_ip = str(shared.get('host'))
          try:
            socket.inet_aton(str(shared.get('host')))
            dip = str(shared.get('host'))
            LOG.info('SET')
            dbase=config.get('sql', 'db')
            duser=config.get('sql', 'user')
            dpass=config.get('sql', 'pass')
            ...
            cur = db.cursor()
            ip = str (shared.get('ipaddr'))
            clustername= config.get('Local','ip')
            cur.execute ("INSERT INTO vms VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (instance['uuid'], 'CB_'+clustername, instance['_display_name'], instance['name'],vlan, mac , ip, host_ip))
            cur.close()
            db.commit()
            db.close()
            dip=config.get('Local', 'ip')
            ...
            cur = db.cursor()
            ip = str (shared.get('ipaddr')) # IP address which is assigned to vm
            clustername= 'CB_'+config.get('Local','ip')
            cur.execute ("INSERT INTO vms VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (instance['uuid'], clustername, instance['_display_name'], instance['name'],vlan, mac , ip, dip))
            cur.close()
            db.commit()
            db.close()

          shared.set('host','notset')
