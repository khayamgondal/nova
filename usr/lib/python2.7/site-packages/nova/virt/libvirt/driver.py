...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _destroy(self, instance):
        try:
            ...
            cluster_name=config.get('nova','local')
            dip= config.get(cluster_name,'ip')
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        if shared.get('vlantag') != "empty" :
          ...
          import urllib2
          total = config.get('nova','clusters')
          for i in range(1, int(total)+1):
             current = "cluster"+str(i)
...
#If host is set its mean request is from some other cluster so we need to write to both local and remote database. If host is not set, the request is local and only local db is written.
          ...
          try:
            ...
            cur.execute ("INSERT INTO vms VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (instance['uuid'], clustername, instance['_display_name'], instance['name'],vlan, mac , ip, str(shared.get('host'))))
            cur.close()
            db.commit()
            db.close()
